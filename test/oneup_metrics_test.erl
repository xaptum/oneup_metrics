...
meter_test() ->
  ...
  oneup_metrics:enable(StatsMap),
  timer:sleep(2000),
  true = [0, 0, 0, 0, 0, 0, 0, 0] == oneup_metrics:get_value([g,b,c1,d1,ref1]),
  ...
  true =OneMinRate == 0,
  true =FiveMinRate == 0,
  true =FifteenMinRate == 0,
  true =HourRate == 0,
  true =DayRate == 0,

  timer:sleep(5000),
  [Counternew, Meannew, InstantRatenew, OneMinRatenew, FiveMinRatenew, FifteenMinRatenew, HourRatenew, DayRatenew] = oneup_metrics:get_value([g,b,c1,d1,ref1]),
  true = Counternew == 0,
  true = InstantRatenew == 200,
  true = OneMinRatenew == tick(1,1000,0),
  true = FiveMinRatenew == tick(5,1000,0),
  true = FifteenMinRatenew == tick(15,1000,0),
  true = HourRatenew == tick(60, 1000, 0),
  true = DayRatenew == tick(1440, 1000, 0),
  true = [0, 0, 0, 0, 0, 0, 0, 0] == oneup_metrics:get_value([g,b,c1,d2,ref2]),
  ...
  timer:sleep(60000),
  [Counter_new, _, InstantRate_new, OneMinRate_new, FiveMinRate_new, _, _, _] = oneup_metrics:get_value([g,b,c1,d1,ref1]),
  true = Counter_new == 0,
  true = InstantRate_new == 0,
  true = OneMinRate_new < 1,
  true = FiveMinRate_new > 2.6,
  true = FiveMinRate_new < 2.8,
  application:stop(oneup_metrics).
...
histogram_test()->
  ...
  StatsMap = oneup_metrics:initial_get_config(),
  oneup_metrics:enable(StatsMap),
  true = {0, 0, 0, 0} == oneup_metrics:get_value([g,b,c1,d1,ref1]),
  oneup_metrics:update(StatsMap, [g,b,c2,d1,ref3],10),
  true = {1, 10, 10, 10} == oneup_metrics:get_value([g,b,c1,d1,ref1]),
  oneup_metrics:update(StatsMap, [g,b,c2,d1,ref3],20),
  true = {2, 15, 10, 20} == oneup_metrics:get_value([g,b,c1,d1,ref1]),
  true = {0, 0, 0, 0} == oneup_metrics:get_value([g,b,c1,d2,ref2]),
  ...
  true = {2, 25, 10, 35} == oneup_metrics:get_value([g,b,c1,d1,ref1]),
  oneup_metrics:reset([g,b,c1,d1,ref1]),
  true = {0, 0, 999999999999, 0} == oneup_metrics:get_value([g,b,c1,d1,ref1]),
  application:stop(oneup_metrics).
